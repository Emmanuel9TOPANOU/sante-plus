    ...
    <main class="flex-1 p-4 md:p-8 pt-24 transition-all duration-300">
        <div class="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-fade-in-up">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-200 pb-8">
                ...
                <div class="flex items-center gap-4">
                    <div class="glass-effect px-6 py-4 rounded-2xl shadow-sm text-center">
                        ...
                        <p class="text-2xl font-black text-blue-600"><?php echo e($medecins->total()); ?></p>
                    </div>
                    <a href="<?php echo e(route('admin.dashboard')); ?>" class="inline-flex items-center gap-2 px-5 py-4 bg-white border border-slate-200 rounded-xl text-xs font-black uppercase tracking-wider text-slate-600 hover:text-blue-600 hover:border-blue-200 hover:shadow-md transition-all duration-300">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
                <form action="<?php echo e(route('admin.medecins.index')); ?>" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    ...
                    <div class="md:col-span-3 flex gap-3">
                        ...
                        <?php if(request()->anyFilled(['search', 'specialite'])): ?>
                            <a href="<?php echo e(route('admin.medecins.index')); ?>" class="px-5 py-3.5 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-all flex items-center justify-center font-black text-[10px] uppercase tracking-wider">
                                actualiser
                            </a>
                        <?php endif; ?>
                    </div>
                        ...
                        <div class="relative p-5 pb-0">
                            <div class="flex justify-between items-start">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 class="font-black text-slate-800 text-base"><?php echo e($medecin->name); ?></h3>
                                        <p class="text-[11px] text-slate-400 font-medium"><?php echo e($medecin->email); ?></p>
                                    </div>
                                </div>
                        ...
                        <div class="p-5 pt-3">
                            <div class="flex items-center justify-between mb-4 pt-2 border-t border-slate-100">
                                <div class="flex items-center gap-2">
                                    <div class="relative">
                                        <div class="w-2.5 h-2.5 rounded-full <?php echo e($medecin->medecin?->est_valide ? 'bg-emerald-500' : 'bg-amber-500'); ?>"></div>
                                        <?php if(!$medecin->medecin?->est_valide): ?>
                                            <div class="absolute inset-0 w-2.5 h-2.5 rounded-full bg-amber-500 animate-ping-slow"></div>
                                        <?php endif; ?>
                                    </div>
                                    <span class="text-[10px] font-black uppercase <?php echo e($medecin->medecin?->est_valide ? 'text-emerald-600' : 'text-amber-600'); ?>">
                                        <?php echo e($medecin->medecin?->est_valide ? 'Compte validé' : 'En attente de validation'); ?>

                                    </span>
                                </div>
                                <span class="text-[10px] text-slate-400">Inscrit le <?php echo e($medecin->created_at->translatedFormat('d M Y')); ?></span>
                            </div>
                        </div>

                     
<div class="p-4 bg-slate-50/50 border-t border-slate-100 space-y-2">
    <?php if($medecin->medecin): ?>
        
        <form action="<?php echo e(route('admin.medecins.validate', ['id' => $medecin->medecin->id])); ?>" method="POST">
            ...
            <button type="submit" 
                    ...
                    <?php echo e($medecin->medecin->est_valide 
        ...
        <form action="<?php echo e(route('admin.medecins.destroy', $medecin->id)); ?>" method="POST" onsubmit="return confirm('⚠️ Attention : Cette action est irréversible. Confirmer la suppression ?')">
