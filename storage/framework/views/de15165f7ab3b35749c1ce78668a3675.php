        ...
        <div class="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-fade-in-up">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-200 pb-8">
                <div>
                    ...
                    <h2 class="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">
                        Gestion des <span class="text-blue-600">Comptes</span>
                    </h2>
                    <p class="text-slate-500 mt-2 text-sm">Gérez tous les utilisateurs de la plateforme</p>
                </div>
                
                <a href="<?php echo e(route('admin.dashboard')); ?>" class="inline-flex items-center gap-2 px-5 py-3 bg-white border border-slate-200 rounded-xl text-xs font-black uppercase tracking-wider text-slate-600 hover:text-blue-600 hover:border-blue-200 hover:shadow-md transition-all duration-300">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                    ...
                    <p class="text-2xl font-black text-slate-800"><?php echo e($users->total()); ?></p>
                </div>
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                    <p class="text-[10px] font-black uppercase tracking-wider text-slate-400">Administrateurs</p>
                    <p class="text-2xl font-black text-purple-600"><?php echo e($users->where('role', 'admin')->count()); ?></p>
                </div>
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                    <p class="text-[10px] font-black uppercase tracking-wider text-slate-400">Médecins</p>
                    <p class="text-2xl font-black text-emerald-600"><?php echo e($users->where('role', 'medecin')->count()); ?></p>
                </div>
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                    <p class="text-[10px] font-black uppercase tracking-wider text-slate-400">Patients</p>
                    <p class="text-2xl font-black text-blue-600"><?php echo e($users->where('role', 'patient')->count()); ?></p>
                </div>
            </div>

            
            <div class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden admin-table-wrapper">
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-slate-50/80 border-b border-slate-100">
                                <th class="px-6 py-5 text-[10px] font-black uppercase tracking-wider text-slate-500">Membre</th>
                                <th class="px-6 py-5 text-[10px] font-black uppercase tracking-wider text-slate-500">Email</th>
                                <th class="px-6 py-5 text-[10px] font-black uppercase tracking-wider text-slate-500">Rôle</th>
                                <th class="px-6 py-5 text-[10px] font-black uppercase tracking-wider text-slate-500 text-center">Statut</th>
                                <th class="px-6 py-5 text-[10px] font-black uppercase tracking-wider text-slate-500">Date</th>
                                <th class="px-6 py-5 text-[10px] font-black uppercase tracking-wider text-slate-500 text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-blue-50/30 transition-all duration-200 group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-100 to-blue-50 flex items-center justify-center text-blue-600 font-bold shadow-sm">
                                ...
                                <td class="px-6 py-4">
                                    <p class="text-slate-500 text-sm"><?php echo e($user->email); ?></p>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-3 py-1 rounded-lg text-[10px] font-black uppercase
                                        <?php echo e($user->role === 'admin' ? 'bg-purple-100 text-purple-600' : ''); ?>

                                        <?php echo e($user->role === 'medecin' ? 'bg-emerald-100 text-emerald-600' : ''); ?>

                                        <?php echo e($user->role === 'patient' ? 'bg-blue-100 text-blue-600' : ''); ?>">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <?php if($user->role === 'medecin'): ?>
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <span class="<?php echo e($user->medecin?->est_valide ? 'text-emerald-600' : 'text-amber-600'); ?> text-[10px] font-black uppercase">
                                                <?php echo e($user->medecin?->est_valide ? 'Validé' : 'En attente'); ?>
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-xs text-slate-500"><?php echo e($user->created_at->translatedFormat('d M Y')); ?></span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <?php if($user->role === 'medecin' && $user->medecin): ?>
                                            <form action="<?php echo e(route('admin.medecins.validate', ['id' => $user->medecin->id])); ?>" method="POST" class="inline">
                                                <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                                <button type="submit" 
                                                        class="px-4 py-2 rounded-xl font-black text-[9px] uppercase tracking-wider transition-all duration-300
                                                        <?php echo e($user->medecin->est_valide ? 'bg-amber-100 text-amber-600 hover:bg-amber-600 hover:text-white' : 'bg-emerald-100 text-emerald-600 hover:bg-emerald-600 hover:text-white'); ?>">
                                                    <?php echo e($user->medecin->est_valide ? 'Suspendre' : 'Valider'); ?>

                                                </button>
                                            </form>
                                        <?php endif; ?>

                                        <form action="<?php echo e(route('admin.users.destroy', $user)); ?>" method="POST" onsubmit="return confirm('Confirmer la suppression de cet utilisateur ?')" class="inline">
                                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                            <button type="submit" class="p-2 rounded-lg text-rose-500 hover:bg-rose-50 transition-all">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
