        ...
        <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between mb-8 gap-6 px-4 md:px-0">
                <div class="flex flex-col sm:flex-row items-center text-center sm:text-left space-y-4 sm:space-y-0 sm:space-x-6">
                    ...
                    <div>
                        <h1 class="text-2xl md:text-3xl font-black text-slate-900 italic tracking-tighter"><?php echo e($patient->name); ?></h1>
                        <p class="text-slate-500 font-bold text-[10px] md:text-xs uppercase tracking-[0.1em] md:tracking-[0.2em] leading-relaxed">
                            ID: #PAT-<?php echo e($patient->id); ?> <span class="hidden sm:inline">•</span><br class="sm:hidden">
                            <?php echo e($patient->date_naissance ? \Carbon\Carbon::parse($patient->date_naissance)->age : '??'); ?> ans • 
                            Sexe: <span class="text-slate-900"><?php echo e($patient->sexe ?? 'N/A'); ?></span> <span class="hidden sm:inline">•</span><br class="sm:hidden">
                            Groupe: <span class="text-red-600 font-black"><?php echo e($patient->groupe_sanguin ?? 'Inconnu'); ?></span>
                        </p>
                    </div>
                </div>
                ...
                <div class="space-y-6 md:space-y-8 order-2 lg:order-1">
                    ...
                    <div class="bg-red-50 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 border border-red-100 shadow-xl shadow-red-100/50 relative overflow-hidden">
                        ...
                        <div class="flex flex-wrap gap-2 relative z-10">
                            <?php if($patient->allergies && $patient->allergies !== 'Aucune'): ?>
                                <?php $__currentLoopData = preg_split('/[, et]+/', $patient->allergies); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $allergy): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <span class="bg-white px-3 py-1.5 rounded-xl text-red-600 text-[9px] font-black uppercase border border-red-200 shadow-sm"><?php echo e(trim($allergy)); ?></span>
                        ...
                        <ul class="space-y-4">
                            <?php if($patient->antecedents): ?>
                                <li class="flex items-start">
                                    ...
                                    <div>
                                        <p class="text-sm font-bold text-slate-800 leading-tight"><?php echo e($patient->antecedents); ?></p>
                                        <p class="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase mt-1"><?php echo e($patient->observations_medicales ?? 'Pas d\'observations globales'); ?></p>
                                    </div>
                                </li>
                            <?php else: ?>
                        ...
                        <div class="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-xl shadow-slate-200/40 group hover:border-blue-200 transition-all">
                            <div class="flex flex-col sm:flex-row justify-between items-start mb-6 gap-4">
                                <div>
                                    <span class="bg-blue-50 text-blue-600 px-4 py-1.5 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest">
                                        <?php echo e($consultation->created_at->translatedFormat('d F Y')); ?>

                                    </span>
                                    <h4 class="text-lg md:text-xl font-black text-slate-900 mt-3 italic"><?php echo e($consultation->diagnostic); ?></h4>
                                </div>
                                ...
                                <div class="flex space-x-2 sm:space-x-4 bg-slate-50 p-3 md:p-4 rounded-2xl w-full sm:w-auto justify-between sm:justify-start">
                                    <div class="text-center px-2">
                                        ...
                                        <p class="text-[11px] md:text-xs font-bold text-blue-600"><?php echo e($consultation->poids ?? '--'); ?> kg</p>
                                    </div>
                                    <div class="text-center border-l border-slate-200 px-2 sm:pl-4">
                                        ...
                                        <p class="text-[11px] md:text-xs font-bold text-blue-600"><?php echo e($consultation->tension ?? '--'); ?></p>
                                    </div>
                                    <div class="text-center border-l border-slate-200 px-2 sm:pl-4">
                                        <p class="text-[8px] md:text-[9px] font-black text-slate-400 uppercase">Temp.</p>
                                        <p class="text-[11px] md:text-xs font-bold text-blue-600"><?php echo e($consultation->temperature ?? '--'); ?>°C</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-50/50 rounded-2xl p-5 md:p-6 mb-6 border border-slate-100/50">
                                <p class="text-xs md:text-sm text-slate-600 font-medium leading-relaxed italic">
                                    "<?php echo e($consultation->observations ?? 'Pas d\'observations pour cette séance.'); ?>"
                                </p>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-center justify-between pt-4 border-t border-slate-50 gap-4">
                                <div class="flex items-center space-x-2 w-full sm:w-auto">
                                    <div class="w-8 h-8 rounded-full bg-blue-100 flex-shrink-0 flex items-center justify-center text-[10px] font-bold text-blue-600 border border-blue-200 uppercase">
                                        <?php echo e(substr($consultation->doctor->name ?? 'D', 0, 1)); ?>

                                    </div>
                                    <span class="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest truncate">Dr. <?php echo e($consultation->doctor->name ?? 'Inconnu'); ?></span>
                                </div>
                                
                                <?php if(isset($consultation->prescription)): ?>
                                    <a href="<?php echo e(route('doctor.prescriptions.show', $consultation->prescription->id)); ?>" class="w-full sm:w-auto text-center border sm:border-none border-blue-100 py-2 sm:py-0 rounded-xl text-blue-600 font-black text-[9px] md:text-[10px] uppercase tracking-widest flex items-center justify-center group-hover:translate-x-1 transition-all">
                                        Voir Ordonnance 
...
<?php endif; ?><?php /**PATH C:\Users\POSTE DETRAVAIL\Desktop\Soutenance\Santé+\resources\views/doctor/patients/show.blade.php ENDPATH**/ ?>