        ...
        <div class="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-fade-in-up">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-200">
                <div>
                    ...
                    <p class="text-slate-500 mt-2 text-sm">{{ \Carbon\Carbon::today()->translatedFormat('l d F Y') }}</p>
                </div>
                ...
                <div class="glass-effect px-6 py-3.5 rounded-2xl shadow-sm text-sm font-medium text-slate-600 border border-white/50 flex items-center gap-2">
                    ...
                    <span>Total: <span class="font-black text-blue-600">{{ $rendezvous->count() }}</span> rendez-vous</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $rendezvous->count() }}</p>
                </div>
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                    <p class="text-[9px] font-black uppercase tracking-wider text-slate-400">Confirmés</p>
                    <p class="text-2xl font-black text-emerald-600">{{ $rendezvous->where('statut', 'confirme')->count() }}</p>
                </div>
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                    <p class="text-[9px] font-black uppercase tracking-wider text-slate-400">En attente</p>
                    <p class="text-2xl font-black text-amber-600">{{ $rendezvous->where('statut', 'en_attente')->count() }}</p>
                </div>
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                    <p class="text-[9px] font-black uppercase tracking-wider text-slate-400">Terminés</p>
                    <p class="text-2xl font-black text-blue-600">{{ $rendezvous->where('statut', 'termine')->count() }}</p>
                </div>
            </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($rendezvous as $rdv)
                    <tr class="agenda-row transition-all duration-200">
                        ...
                        <td class="px-6 py-5">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-black text-lg shadow-md">
                                    {{ substr($rdv->patient->name ?? 'P', 0, 1) }}{{ substr($rdv->patient->name ?? 'P', 1, 1) ?? '' }}
                                </div>
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-sm font-black text-slate-800">{{ $rdv->patient->name ?? 'Patient' }}</span>
                                        <span class="px-2 py-0.5 bg-slate-100 text-slate-500 rounded text-[9px] font-black uppercase">ID: #{{ $rdv->patient_id }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg">{{ $rdv->heure_rdv }}</span>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5">
                            <div class="flex flex-col gap-0.5">
                                ...
                                <div class="flex items-center gap-1 text-slate-600 text-xs">
                                    ...
                                    {{ $rdv->patient->patient?->telephone ?? 'Non renseigné' }}
                                </div>
                            </div>
                        </td>
                        ...
                        {{-- Statut --}}
                        <td class="px-6 py-5">
                            @if($rdv->statut === 'attente') {{-- Corrigé : attente --}}
                        ...
                        <td class="px-6 py-5 text-right">
                            <div class="flex justify-end items-center gap-2">
                                ...
                                {{-- 3. BOUTON ANNULER --}}
                                @if($rdv->statut === 'attente' || $rdv->statut === 'confirme') {{-- Corrigé : attente --}}
                                ...
                                {{-- 4. BOUTON DÉMARRER CONSULTATION --}}
