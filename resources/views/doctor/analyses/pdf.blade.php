...
<body>
        ...
        <table class="info-table">
            <tr>
                <td class="info-td">
                    ...
                    <span class="value">{{ $analyse->user->name }}</span><br>
                    ID : #{{ str_pad($analyse->user->id, 6, '0', STR_PAD_LEFT) }}<br>
                    Sexe : {{ $analyse->user->sexe ?? 'Non spécifié' }}
                </td>
                <td class="info-td" style="text-align: right;">
                    <span class="label">Détails Examen</span><br>
                    Prélèvement : <span class="value">{{ \Carbon\Carbon::parse($analyse->date_prelevement)->format('d/m/Y H:i') }}</span><br>
                    Émis le : {{ now()->format('d/m/Y') }}<br>
                    Laboratoire : <span class="value">{{ $analyse->laboratoire_nom ?? 'Interne' }}</span>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="results-table">
        ...
        <tbody>
            <tr>
                <td style="font-weight: bold; color: #0f172a;">{{ $analyse->type_analyse }}</td>
                <td style="text-align: center;" class="result-value">{{ $analyse->valeur }}</td>
                <td style="text-align: center; color: #64748b;">{{ $analyse->unite }}</td>
                <td style="text-align: center; color: #64748b; font-family: monospace;">{{ $analyse->norme }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="interpretation-box">
        ...
        <p style="margin: 0; color: #334155; font-style: italic;">
            "{{ $analyse->interpretation ?? 'Les résultats doivent être interprétés par votre médecin traitant.' }}"
        </p>
    </div>

    <div class="footer">
        <div class="signature-block">
            <div class="signature-text">Dr. {{ $analyse->biologiste_nom }}</div>
            <div style="font-size: 11px; color: #2563eb;">Biologiste Responsable</div>
            <div class="validation-stamp">
                Validé électroniquement le {{ \Carbon\Carbon::parse($analyse->date_validation ?? now())->format('d/m/Y à H:i') }}
            </div>
        </div>
