        ...
        <div class="max-w-7xl mx-auto px-4 md:px-8">
            <div class="flex justify-between items-center h-20">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-bold text-slate-800 tracking-tight">Espace<span class="text-blue-600">Médecin</span></h1>
                        <p class="text-[9px] text-slate-400 font-semibold uppercase tracking-wider">Dr. {{ Auth::user()->name }}</p>
                    </div>
                </div>
    ...
    <main class="flex-1 p-4 md:p-8 pt-24 transition-all duration-300">
        <div class="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-fade-in-up">
            ...
            <nav class="flex items-center space-x-2 text-[10px] font-black uppercase tracking-wider">
                ...
                <span class="text-slate-300">/</span>
                <span class="text-blue-600">Dossier</span>
            </nav>

            {{-- HEADER PATIENT PREMIUM --}}
            <div class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
                <div class="relative bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-8 md:px-8">
                    <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div class="flex flex-col sm:flex-row items-center text-center sm:text-left gap-5">
                            <div class="w-24 h-24 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center text-white text-3xl font-black shadow-lg border border-white/30">
                                @php
                            ...
                            <div>
                                <h1 class="text-2xl md:text-3xl font-black text-white">{{ $patient->name }}</h1>
                                <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2 mt-2">
                                    <span class="px-2 py-1 bg-white/20 rounded-lg text-white text-[10px] font-black uppercase">ID: #PAT-{{ $patient->id }}</span>
                                    <span class="px-2 py-1 bg-white/20 rounded-lg text-white text-[10px] font-black uppercase">{{ $patient->age ? $patient->age . ' ans' : 'Âge inconnu' }}</span>
                                    <span class="px-2 py-1 bg-white/20 rounded-lg text-white text-[10px] font-black uppercase">{{ $patient->sexe ?? 'Sexe inconnu' }}</span>
                                    <span class="px-2 py-1 bg-rose-500/80 rounded-lg text-white text-[10px] font-black uppercase">{{ $patient->groupe_sanguin ?? 'Groupe inconnu' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-3">
                            <a href="{{ route('doctor.patients.edit', $patient->id) }}" 
                               class="px-5 py-2.5 bg-white text-blue-600 rounded-xl font-black text-[11px] uppercase tracking-wider hover:bg-blue-50 transition-all flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('doctor.patients.index') }}" 
                               class="px-5 py-2.5 bg-white/20 text-white rounded-xl font-black text-[11px] uppercase tracking-wider hover:bg-white/30 transition-all flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- Contact Rapide --}}
                <div class="px-6 py-4 bg-slate-50 border-b border-slate-100 flex flex-wrap items-center gap-4">
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                        </svg>
                        <span class="text-sm text-slate-600">{{ $patient->email }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                        </svg>
                        <span class="text-sm text-slate-600">{{ $patient->patient?->telephone ?? 'Non renseigné' }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <span class="text-sm text-slate-600">{{ $patient->patient?->adresse ?: 'Adresse non renseignée' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {{-- CARTE ALLERGIES --}}
                <div class="bg-gradient-to-br from-rose-50 to-white rounded-2xl border border-rose-100 p-6 shadow-md">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h3 class="font-black text-slate-800">Allergies</h3>
                    </div>
                    <div class="flex flex-wrap gap-2">
                        @if($patient->patient?->allergies && $patient->patient->allergies !== 'Aucune')
                            @foreach(preg_split('/[, et]+/', $patient->patient->allergies) as $allergy)
                                <span class="px-3 py-1.5 bg-rose-100 text-rose-700 rounded-xl text-[11px] font-bold">{{ trim($allergy) }}</span>
                            @endforeach
                        @else
                            <p class="text-slate-400 text-sm italic">Aucune allergie signalée</p>
                        @endif
                    </div>
                </div>

                {{-- CARTE ANTÉCÉDENTS --}}
                <div class="bg-gradient-to-br from-amber-50 to-white rounded-2xl border border-amber-100 p-6 shadow-md">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h3 class="font-black text-slate-800">Antécédents</h3>
                    </div>
                    <p class="text-slate-700 text-sm leading-relaxed">
                        {{ $patient->patient?->antecedents ?? 'Aucun antécédent médical renseigné' }}
                    </p>
                </div>

                {{-- CARTE CONTACT URGENCE --}}
                <div class="bg-gradient-to-br from-blue-50 to-white rounded-2xl border border-blue-100 p-6 shadow-md">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h3 class="font-black text-slate-800">Informations complémentaires</h3>
                    </div>
                    <div class="space-y-2">
                        <p class="text-sm text-slate-600">
                            <span class="font-bold">Date création dossier:</span><br>
                            {{ $patient->created_at->translatedFormat('d F Y') }}
                        </p>
                        <p class="text-sm text-slate-600">
                            <span class="font-bold">Dernière consultation:</span><br>
                            {{ $consultations->first()?->created_at->translatedFormat('d F Y') ?? 'Aucune' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <h3 class="text-slate-800 font-black text-lg">Historique des consultations</h3>
                    <span class="text-slate-400 text-[10px] font-black uppercase tracking-wider">{{ $consultations->count() }} consultation(s)</span>
                </div>

                @forelse($consultations as $consultation)
                    <div class="consultation-card bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-md hover:shadow-lg">
                        <div class="p-5 md:p-6">
                            <div class="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4">
                                <div>
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-100 text-blue-700 rounded-xl text-[10px] font-black">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                        </svg>
                                        {{ $consultation->created_at->translatedFormat('d F Y - H:i') }}
                                    </span>
                                    <h4 class="text-lg font-black text-slate-800 mt-2">{{ $consultation->diagnostic ?? 'Consultation médicale' }}</h4>
                                </div>
                                
                                <div class="flex gap-3 bg-slate-50 p-2 rounded-xl">
                                    <div class="text-center px-3">
                                        <p class="text-[8px] font-black text-slate-400 uppercase">Poids</p>
                                        <p class="text-sm font-bold text-blue-600">{{ $consultation->poids ?? '--' }} kg</p>
                                    </div>
                                    <div class="text-center px-3 border-l border-slate-200">
                                        <p class="text-[8px] font-black text-slate-400 uppercase">Tension</p>
                                        <p class="text-sm font-bold text-blue-600">{{ $consultation->tension ?? '--' }}</p>
                                    </div>
                                    <div class="text-center px-3 border-l border-slate-200">
                                        <p class="text-[8px] font-black text-slate-400 uppercase">Temp.</p>
                                        <p class="text-sm font-bold text-blue-600">{{ $consultation->temperature ?? '--' }}°C</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-50 rounded-xl p-4 mb-4">
                                <p class="text-slate-600 text-sm italic">
                                    "{{ $consultation->observations ?? 'Aucune observation pour cette consultation.' }}"
                                </p>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row justify-between items-center gap-3 pt-2 border-t border-slate-100">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[10px] font-black text-slate-500 uppercase">Dr. {{ $consultation->doctor->name ?? 'Inconnu' }}</span>
                                </div>
                                
                                @if(isset($consultation->prescription))
                                    <a href="{{ route('doctor.prescriptions.show', $consultation->prescription->id) }}" 
                                       class="text-blue-600 font-black text-[10px] uppercase tracking-wider hover:text-blue-700 transition-all flex items-center gap-1">
                                        Voir l'ordonnance
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @empty
                    <div class="bg-white rounded-2xl p-12 text-center border-2 border-dashed border-slate-200">
                        ...
                        <p class="text-slate-400 font-bold text-sm">Aucune consultation enregistrée</p>
                        <p class="text-slate-300 text-xs mt-1">Les consultations apparaîtront ici</p>
                    </div>
                @endforelse
            </div>
