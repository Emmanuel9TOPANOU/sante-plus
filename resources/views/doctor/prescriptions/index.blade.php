    ...
    <div class="py-6 md:py-12 bg-[#F8FAFC] min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8">
                <a href="{{ route('dashboard') }}" class="group inline-flex items-center text-gray-400 hover:text-slate-900 transition-all">
                ...
                <div class="mb-8 p-5 md:p-6 bg-white border-l-4 border-emerald-500 rounded-2xl shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <span class="font-bold text-gray-800 text-sm">{{ session('success') }}</span>
                        </div>
                    </div>
                    ...
                    @if(session('download'))
                        ...
                        <a href="{{ session('download') }}" class="w-full md:w-auto text-center bg-emerald-50 text-emerald-600 px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition border border-emerald-100">
                            Relancer le PDF
                        </a>
                        @php session()->forget('download'); @endphp
                    @endif
                </div>
            @endif
            ...
            <div class="bg-white shadow-2xl shadow-slate-200/50 rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden border border-gray-100">
                ...
                <div class="block lg:hidden divide-y divide-gray-50">
                        ...
                        <div class="p-6 bg-white hover:bg-blue-50/20 transition-all">
                            <div class="flex justify-between items-start mb-4">
                                ...
                                <span class="text-[10px] font-bold text-gray-400">
                                    {{ \Carbon\Carbon::parse($p->date_emission)->translatedFormat('d M Y') }}
                                </span>
                            </div>
                            ...
                            <div class="flex items-center mb-6">
                                <div class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center mr-3 text-slate-500 text-xs font-black ring-2 ring-slate-50">
                                    {{ strtoupper(substr($p->patient->name, 0, 2)) }}
                                </div>
                                <span class="text-sm font-black text-gray-700 uppercase">{{ $p->patient->name }}</span>
                            </div>

                            <a href="{{ route('doctor.prescriptions.download', $p->id) }}" 
                    ...
                    @empty
                        {{-- Même contenu vide que plus bas --}}
                    @endforelse
                </div>
                                ...
                                <tr class="group hover:bg-blue-50/30 transition-all">
                                    ...
                                    <td class="py-8 px-6">
                                        <div class="flex items-center">
                                            ...
                                            <span class="text-sm font-black text-gray-700 uppercase tracking-tight">{{ $p->patient->name }}</span>
                                        </div>
                                    </td>
                                    <td class="py-8 px-6">
