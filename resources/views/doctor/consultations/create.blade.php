    ...
    <main class="flex-1 p-4 md:p-8 pt-24 transition-all duration-300">
        <div class="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-fade-in-up">
            ...
            <nav class="flex items-center space-x-2 text-[10px] font-black uppercase tracking-wider">
                ...
                <a href="{{ route('doctor.rendezvous.index') }}" class="text-slate-400 hover:text-blue-600 transition-colors">Agenda</a>
                <span class="text-slate-300">/</span>
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            ...
                            <p class="text-blue-200 text-[10px] font-black uppercase tracking-wider mt-1">
                                Patient : {{ $rendezvous->patient->name }} | ID: #{{ $rendezvous->patient->id }}
                            </p>
                        </div>
                        <div class="text-left sm:text-right">
                            <span class="block text-[9px] font-black text-blue-200 uppercase tracking-wider">Date de session</span>
                            <span class="font-bold text-sm text-white">{{ now()->translatedFormat('d F Y') }}</span>
                        </div>
                    </div>
                </div>

                <form action="{{ route('doctor.consultations.store') }}" method="POST" class="p-6 md:p-8">
                    @csrf
                        ...
                        <div class="space-y-5 bg-slate-50/50 p-5 rounded-xl border border-slate-100 h-fit lg:sticky lg:top-28">
                                ...
                                <div>
                                    ...
                                    <select name="groupe_sanguin" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-xs font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                                        ...
                                        @foreach(['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'] as $gp)
                                            <option value="{{ $gp }}" {{ $rendezvous->patient->groupe_sanguin == $gp ? 'selected' : '' }}>{{ $gp }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div id="age_display" class="w-full bg-blue-100 rounded-xl p-3 font-black text-blue-700 text-sm text-center mb-2">
                                    {{ $rendezvous->patient->date_naissance ? \Carbon\Carbon::parse($rendezvous->patient->date_naissance)->age . ' ans' : 'Non renseigné' }}
                                </div>
                                <input type="date" name="date_naissance" id="birth_date" value="{{ $rendezvous->patient->date_naissance ? \Carbon\Carbon::parse($rendezvous->patient->date_naissance)->format('Y-m-d') : '' }}" 
                            ...
                            <div>
                                ...
                                <textarea name="antecedents_medicaux" rows="3" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-xs font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all resize-none" placeholder="Diabète, Hypertension, etc...">{{ $rendezvous->patient->antecedents_medicaux }}</textarea>
                            </div>

                            <div>
                                <label class="block text-[9px] font-black text-rose-500 mb-1 uppercase">Allergies</label>
                                <textarea name="allergies" rows="2" class="w-full bg-white border border-rose-200 rounded-xl px-3 py-2.5 text-xs font-medium text-rose-600 focus:ring-2 focus:ring-rose-500 focus:border-rose-500 outline-none transition-all resize-none" placeholder="Aucune allergie connue">{{ $rendezvous->patient->allergies }}</textarea>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col-reverse sm:flex-row justify-end items-center gap-4 border-t border-slate-100 mt-8 pt-8">
                        <a href="{{ route('doctor.dashboard') }}" class="text-[10px] font-black uppercase text-slate-400 hover:text-rose-500 tracking-wider py-2 transition-colors">Annuler</a>
