    ...
    <main class="flex-1 p-4 md:p-8 pt-24 transition-all duration-300">
        <div class="max-w-7xl mx-auto space-y-6 md:space-y-8">
            ...
            <nav class="flex items-center space-x-2 text-[10px] font-black uppercase tracking-wider">
                ...
                <a href="{{ route('doctor.messages.index') }}" class="text-slate-400 hover:text-blue-600 transition-colors">Messagerie</a>
                <span class="text-slate-300">/</span>
    ...
    <div class="flex flex-col h-full">
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-1 scrollbar-thin">
            ...
            <a href="{{ route('doctor.messages.show', $secretaire->id) }}" 
               ...
               class="contact-item flex items-center gap-3 p-3 rounded-xl transition-all duration-200 {{ isset($activeContact) && $activeContact->id == $secretaire->id ? 'bg-blue-50 border-l-4 border-blue-600' : 'hover:bg-slate-100' }}">
                ...
                <div class="flex-1 min-w-0">
                    <p class="font-semibold text-slate-800 text-sm truncate">{{ $secretaire->name }}</p>
                    <p class="text-[9px] text-slate-400 font-medium uppercase tracking-wider">Secrétariat</p>
                </div>
                @if(isset($activeContact) && $activeContact->id == $secretaire->id)
            ...
            @foreach($contacts as $contact)
                ...
                <a href="{{ route('doctor.messages.show', $contact->id) }}" 
                   ...
                   class="contact-item flex items-center gap-3 p-3 rounded-xl transition-all duration-200 {{ isset($activeContact) && $activeContact->id == $contact->id ? 'bg-blue-50 border-l-4 border-blue-600' : 'hover:bg-slate-100' }}">
                    <div class="relative">
                        ...
                        @if(($contact->unread ?? 0) > 0)
                            <span class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">{{ $contact->unread }}</span>
                        @endif
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="font-semibold text-slate-800 text-sm truncate">{{ $contact->name }}</p>
                    ...
                    <div class="flex-1 flex flex-col">
                        @if(isset($activeContact))
                                ...
                                @forelse($messages as $message)
                                    ...
                                    <div class="flex {{ $isMe ? 'justify-end' : 'justify-start' }} animate-fade-in-up">
                                        <div class="max-w-[85%] md:max-w-[70%]">
                                            <div class="px-4 py-2.5 rounded-2xl {{ $isMe ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-white border border-slate-200 text-slate-700 rounded-bl-sm shadow-sm' }}">
                                                ...
                                                @if($message->file_path)
                                                    <div class="mt-2 pt-2 border-t {{ $isMe ? 'border-white/20' : 'border-slate-100' }}">
                                                        <a href="{{ asset('storage/' . $message->file_path) }}" target="_blank" 
                                                           class="text-[10px] font-black uppercase tracking-wider flex items-center gap-1 {{ $isMe ? 'text-blue-200' : 'text-blue-600' }}">
                                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <p class="text-[9px] text-slate-400 mt-1 px-2 {{ $isMe ? 'text-right' : 'text-left' }}">
                            ...
                            <div class="p-4 border-t border-slate-100 bg-white">
                                <form action="{{ route('doctor.messages.store') }}" method="POST" enctype="multipart/form-data" class="flex gap-2">
                                    @csrf
