<x-app-layout>
    <div class="py-6 md:py-12 bg-[#F8FAFC] min-h-screen montserrat">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($prescriptionsEnAttente->count() > 0)
                ...
                <div class="hidden md:block overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="hover:bg-blue-50/30 transition-all group">
                                    <td class="px-8 py-6">
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div>
                                                <p class="text-sm font-black text-gray-800 leading-none mb-1">{{ $analyse->type_analyse }}</p>
                                                <p class="text-[10px] font-bold text-blue-500 uppercase tracking-widest">{{ $analyse->created_at->translatedFormat('d M Y') }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-8 py-6 text-center">
                                        @if($status == 'termine' && $analyse->valeur)
                                            <div class="inline-block px-4 py-2 bg-gray-50 rounded-2xl border border-gray-100">
                                                <span class="text-sm font-black text-gray-900">{{ $analyse->valeur }} <span class="text-blue-500 text-[10px]">{{ $analyse->unite }}</span></span>
                                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-tighter">Norme: {{ $analyse->norme ?? 'N/A' }}</p>
                                            </div>
                                        @else
                                    ...
                                    <td class="px-8 py-6">
                                        <span class="px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border {{ $statusClasses }} shadow-sm">
                                            {{ str_replace('_', ' ', $status) }}
                                        </span>
                                    </td>
                                    <td class="px-8 py-6 text-right">
                                        @if($status == 'termine')
                                            {{-- CORRECTION ICI : Route patient pour éviter la 403 --}}
                                            <a href="{{ route('patient.lab_results.download', $analyse->id) }}" class="inline-flex items-center gap-2 bg-slate-900 hover:bg-blue-600 text-white px-5 py-2.5 rounded-xl text-[9px] font-black uppercase transition-all transform hover:-translate-y-1 shadow-lg shadow-slate-100 active:scale-95">
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ $isSpecialist ? 5 : 4 }}" class="px-8 py-32 text-center">
                        ...
                        <div class="p-6">
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <h4 class="text-sm font-black text-gray-800 leading-tight">{{ $analyse->type_analyse }}</h4>
                                        <p class="text-[10px] font-bold text-blue-500 uppercase">{{ $analyse->created_at->translatedFormat('d F Y') }}</p>
                                    </div>
                                </div>
                                <span class="px-2 py-1 rounded-lg text-[8px] font-black uppercase {{ $statusClasses }}">
                                    {{ str_replace('_', ' ', $status) }}
                                </span>
                            </div>
                            ...
                            <div class="bg-gray-50/50 rounded-2xl p-4 flex justify-between items-center border border-gray-100 mb-4">
                                @if($status == 'termine')
                                    <div>
                                        ...
                                        <p class="text-xs font-black text-gray-900">{{ $analyse->valeur }} <span class="text-[9px] text-gray-400 font-normal">{{ $analyse->unite }}</span></p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-[8px] text-gray-400 font-bold uppercase">Norme</p>
                                        <p class="text-[10px] font-bold text-gray-700">{{ $analyse->norme ?? 'N/A' }}</p>
                                    </div>
                                @else
                            ...
                            @if($status == 'termine')
                                {{-- CORRECTION ICI : Route patient --}}
                                <a href="{{ route('patient.lab_results.download', $analyse->id) }}" class="w-full flex items-center justify-center space-x-2 bg-slate-900 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-transform">
