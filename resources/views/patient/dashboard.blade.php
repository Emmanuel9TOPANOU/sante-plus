<x-app-layout>
{{-- Modification de x-data : suppression de showNoteModal car le bouton a été enlevé précédemment --}}
<div class="flex min-h-screen bg-[#FDFDFD]" x-data="{ mobileMenuOpen: false }">

    {{-- BOUTON HAMBURGER (Teinte Sky) --}}
    ...
    {{-- SIDEBAR (Fond blanc uni, accents Sky) --}}
<aside :class="mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
        ...
        <div>
            ...
            <div class="flex flex-col gap-1">
                <a href="{{ route('patient.dashboard') }}"
                    class="flex items-center px-4 py-3 rounded-xl transition-all duration-300 no-underline {{ request()->routeIs('patient.dashboard') ? 'bg-blue-600 text-white shadow-md' : 'text-blue-700 hover:bg-blue-50 hover:text-blue-600' }}">
                ...
                <a href="{{ route('patient.rendezvous.index') }}"
                    class="flex items-center px-4 py-3 rounded-xl transition-all duration-300 no-underline {{ request()->routeIs('patient.rendezvous*') ? 'bg-blue-600 text-white shadow-md' : 'text-blue-700 hover:bg-blue-50 hover:text-blue-600' }}">
                ...
                <a href="{{ route('patient.lab_results.index') }}"
                    class="flex items-center px-4 py-3 rounded-xl transition-all duration-300 no-underline {{ request()->routeIs('patient.lab_results*') ? 'bg-blue-600 text-white shadow-md' : 'text-blue-700 hover:bg-blue-50 hover:text-blue-600' }}">
                ...
                <a href="{{ route('patient.messages.index') }}"
                    class="flex items-center px-4 py-3 rounded-xl transition-all duration-300 no-underline {{ request()->routeIs('patient.messages*') ? 'bg-blue-600 text-white shadow-md' : 'text-blue-700 hover:bg-blue-50 hover:text-blue-600' }}">
        ...
        <div>
            ...
            <div class="flex flex-col gap-1">
                <a href="{{ route('patient.medical_record.index') }}"
                    class="flex items-center px-4 py-3 rounded-xl transition-all duration-300 no-underline {{ request()->routeIs('patient.medical_record*') ? 'bg-blue-600 text-white shadow-md' : 'text-blue-700 hover:bg-blue-50 hover:text-blue-600' }}">
                ...
                <a href="{{ route('patient.prescriptions.index') }}"
                    class="flex items-center px-4 py-3 rounded-xl transition-all duration-300 no-underline {{ request()->routeIs('patient.prescriptions*') ? 'bg-blue-600 text-white shadow-md' : 'text-blue-700 hover:bg-blue-50 hover:text-blue-600' }}">
    ...
    {{-- FOOTER SIDEBAR : Bouton Déconnexion Premium --}}
...
<main class="flex-1 lg:ml-72 p-4 md:p-10 bg-[#FDFDFD] min-h-screen">
    <div class="max-w-7xl mx-auto space-y-8 md:space-y-12">
        ...
        {{-- STATS MODERNES (Bordures Bleues, Style Premium) --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div class="bg-white p-7 rounded-[2rem] shadow-sm border border-gray-100 border-t-4 border-t-blue-600 flex flex-col items-start hover:shadow-xl transition-all duration-300 group">
                ...
                <h3 class="text-4xl font-black text-slate-900">{{ $prochainsRendezVous->count() }}</h3>
            </div>
            {{-- Ordonnances --}}
            <div class="bg-white p-7 rounded-[2rem] shadow-sm border border-gray-100 border-t-4 border-t-blue-500 flex flex-col items-start hover:shadow-xl transition-all duration-300 group">
                ...
                <h3 class="text-4xl font-black text-slate-900">{{ $dernieresOrdonnances->count() }}</h3>
            </div>
            {{-- Analyses --}}
            <div class="bg-white p-7 rounded-[2rem] shadow-sm border border-gray-100 border-t-4 border-t-blue-400 flex flex-col items-start hover:shadow-xl transition-all duration-300 group">
                ...
                <h3 class="text-4xl font-black text-slate-900">{{ $analyses ?? 0 }}</h3>
            </div>
            {{-- ID Patient --}}
            <div class="bg-white p-7 rounded-[2rem] shadow-sm border border-gray-100 border-t-4 border-t-slate-800 flex flex-col items-start hover:shadow-xl transition-all duration-300">
                ...
                <h3 class="text-lg font-black mt-2 text-white bg-slate-800 px-4 py-1 rounded-xl shadow-inner">
                    #HS-{{ str_pad($patient->id ?? 0, 4, '0', STR_PAD_LEFT) }}
                </h3>
            </div>
        </div>

        {{-- ACTIONS RAPIDES (Boutons Style Premium) --}}
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <a href="{{ route('patient.rendezvous.create') }}" class="group flex flex-col items-center justify-center p-6 bg-white border border-blue-50 text-blue-700 rounded-[2rem] font-black hover:bg-blue-600 hover:text-white transition-all shadow-md no-underline text-center">
            ...
            <a href="{{ route('patient.prescriptions.index') }}" class="group flex flex-col items-center justify-center p-6 bg-white border border-blue-50 text-blue-700 rounded-[2rem] font-black hover:bg-blue-600 hover:text-white transition-all shadow-md no-underline text-center">
                <div class="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center mb-3 group-hover:bg-blue-500 transition-colors">
            ...
            <a href="{{ route('patient.lab_results.index') }}" class="group flex flex-col items-center justify-center p-6 bg-white border border-blue-50 text-blue-700 rounded-[2rem] font-black hover:bg-blue-600 hover:text-white transition-all shadow-md no-underline text-center">
                <div class="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center mb-3 group-hover:bg-blue-500 transition-colors">
            ...
            <a href="{{ route('patient.history.index') }}" class="group flex flex-col items-center justify-center p-6 bg-white border border-slate-200 text-slate-700 rounded-[2rem] font-black hover:bg-slate-800 hover:text-white transition-all shadow-md no-underline text-center">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="bg-white rounded-[2.5rem] p-8 shadow-lg border border-gray-50">
                    <div class="flex justify-between items-center mb-10">
                        ...
                        <span class="bg-blue-600 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase shadow-md">
                            {{ $prochainsRendezVous->count() }} RDV planifiés
                        </span>
                    </div>
                                    ...
                                    <tr class="group hover:bg-blue-50/50 transition-all">
                                        <td class="py-6 px-6 md:px-0">
                                            <div class="flex flex-col">
                                                <span class="text-sm font-black text-slate-800 tracking-tight">{{ \Carbon\Carbon::parse($rdv->date_rdv)->translatedFormat('d M Y') }}</span>
                                                <span class="text-[11px] text-white font-black uppercase bg-blue-500 px-3 py-1 rounded-lg self-start mt-2 shadow-sm">{{ $rdv->heure_rdv }}</span>
                                            </div>
                                        </td>
                                        <td class="py-6">
                                            <div class="flex items-center">
                                                <div class="w-11 h-11 rounded-[1rem] bg-gradient-to-br from-blue-100 to-blue-50 flex items-center justify-center mr-4 text-xs font-black text-blue-600 border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition-all">
                                                    {{ substr($rdv->medecin->name ?? 'M', 0, 1) }}
                                                </div>
                                                <span class="text-sm font-bold text-slate-800">Dr. {{ $rdv->medecin->name ?? 'Médecin' }}</span>
                                            </div>
                                        </td>
                                        <td class="py-6">
                                            <span class="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-xl border border-blue-100">
                                                {{ $rdv->medecin->specialite->nom_specialite ?? 'Généraliste' }}
                                            </span>
                                        </td>
            ...
            {{-- COLONNE DE DROITE (Suivi Médical Premium) --}}
                ...
                <div class="bg-gradient-to-b from-blue-600 to-blue-800 text-white rounded-[3rem] p-8 shadow-2xl h-full flex flex-col">
                    ...
                    <div class="space-y-8 flex-1">
                        ...
                        <div class="p-6 bg-white/10 backdrop-blur-md rounded-[2rem] border border-white/20 shadow-xl">
                            ...
                            @if($dernieresOrdonnances->count() > 0)
                                <div class="flex items-center gap-5">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-[11px] font-black text-blue-800 bg-white px-3 py-1 rounded-lg uppercase mt-2 inline-block shadow-sm">
                                            {{ \Carbon\Carbon::parse($dernieresOrdonnances->first()->created_at)->translatedFormat('d F Y') }}
                                        </span>
                                    </div>
                                </div>
                            @else
